    ...
    <section id="working-experience" className="mt-12 lg:mt-[100px]">
      ...
      <ul className="m-0 p-0">
        {experience.map((val, index) => (
          <li
            className="list-none align-baseline border-y border-white p-6 lg:flex hover:bg-primary hover:border-y hover:border-white mb-5 lg:mb-10"
            key={`working-experience-${index}`}
          >
            <div className="max-w-[270px] w-full  flex-1">
              <h3 className="mr-4 mb-2">{val.company}</h3>
              <p>{val.workingPeriod}</p>
            </div>
            <div className="flex-1 lg:pl-6 lg:border-l lg:border-black">
              <h3 className="mb-2">{val.role}</h3>
              <p>{val.description}</p>
            </div>
          </li>
        ))}
